...
// .spec.md §4.2 — Chip. Named `Chip`, not `Tag`, and built directly on the
...
export interface ChipProps extends React.ComponentProps<typeof ChipRoot> {
  /** Renders the 19px circular remove control (.spec.md §4.2). */
  onRemove?: () => void;
}

const ChipRoot = withProvider<HTMLSpanElement, { colorPalette?: string; children?: React.ReactNode }>(
  "span",
  "root",
);
const ChipLabel = withContext<HTMLSpanElement, { children?: React.ReactNode }>("span", "label");
const ChipCloseTrigger = withContext<HTMLButtonElement, React.ButtonHTMLAttributes<HTMLButtonElement>>(
  "button",
  "closeTrigger",
);

export const Chip = React.forwardRef<HTMLSpanElement, ChipProps>(({ children, onRemove, ...props }, ref) => (
